  ...
  return (
    <div className="min-h-screen bg-[#f9f5f6] py-8 px-4">
      <div className="max-w-7xl mx-auto">
        ...
        {/* Table */}
        <div className="bg-white rounded-lg border-4 border-black overflow-hidden" style={{ boxShadow: '8px 8px 0px 0px #000' }}>
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead className="bg-gray-100 border-b-2 border-black">
                <tr>
                  <th className="px-4 py-3 text-left text-sm font-black">Title</th>
                  <th className="px-4 py-3 text-left text-sm font-black">Category</th>
                  <th className="px-4 py-3 text-left text-sm font-black">Visibility</th>
                  <th className="px-4 py-3 text-left text-sm font-black">Access</th>
                  <th className="px-4 py-3 text-left text-sm font-black">Stats</th>
                  <th className="px-4 py-3 text-left text-sm font-black">Created</th>
                  <th className="px-4 py-3 text-center text-sm font-black">Actions</th>
                </tr>
              </thead>
              <tbody>
                {lessons.map((lesson) => (
                  <tr key={lesson._id} className="border-b-2 border-gray-200 hover:bg-gray-50">
                    <td className="px-4 py-4">
                      <p className="font-bold text-gray-900">{lesson.title}</p>
                      <p className="text-xs text-gray-500">{lesson.emotionalTone}</p>
                    </td>
                    <td className="px-4 py-4">
                    ...
                    <td className="px-4 py-4">
                      <button
                        onClick={() => toggleVisibility(lesson._id, lesson.privacy)}
                    ...
                    <td className="px-4 py-4">
                      <button
                        onClick={() => toggleAccessLevel(lesson._id, lesson.accessLevel)}
                        disabled={!isPremium}
                        className={`px-3 py-1 text-xs font-bold rounded-full border-2 ${
                    ...
                    <td className="px-4 py-4">
                      <p className="text-xs text-gray-600">❤️ {lesson.likesCount}</p>
                      <p className="text-xs text-gray-600">🔖 {lesson.favoritesCount}</p>
                    </td>
                    ...
                    <td className="px-4 py-4">
                      <div className="flex gap-2 justify-center">
                        <button
                          onClick={() => window.open(`/lesson/${lesson._id}`, '_blank')}
          ...
          {lessons.length === 0 && (
            <div className="text-center py-12">
              <p className="text-2xl font-bold text-gray-400 mb-4">No lessons yet</p>
      ...
      {showUpdateModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg border-4 border-black p-6 max-w-2xl w-full max-h-[90vh] overflow-y-auto" style={{ boxShadow: '8px 8px 0px 0px #000' }}>
            ...
            <form onSubmit={(e) => {
              ...
              setLessons(lessons.map(l => 
                l._id === selectedLesson._id ? { ...l, ...updatedLesson } : l
              ));
              ...
              <div className="grid grid-cols-2 gap-4 mb-4">
                ...
                <div>
                  <label className="block text-sm font-bold mb-2">Emotional Tone *</label>
                  <select
                    value={updateFormData.emotionalTone}
                    onChange={(e) => setUpdateFormData({...updateFormData, emotionalTone: e.target.value})}
                    className="w-full px-4 py-2 border-2 border-gray-300 rounded-lg"
                    required
                  >
              ...
              <div className="grid grid-cols-2 gap-4 mb-6">
                <div>
                  <label className="block text-sm font-bold mb-2">Privacy *</label>
                  <select
                    value={updateFormData.privacy}
                    onChange={(e) => setUpdateFormData({...updateFormData, privacy: e.target.value})}
                    className="w-full px-4 py-2 border-2 border-gray-300 rounded-lg"
                    required
                  >
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => setUpdateFormData({...updateFormData, accessLevel: e.target.value})}
                    disabled={!isPremium}
                    className={`w-full px-4 py-2 border-2 border-gray-300 rounded-lg ${!isPremium ? 'bg-gray-100' : ''}`}
                    required
                  >
      ...
      {showDeleteModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg border-4 border-black p-6 max-w-md w-full" style={{ boxShadow: '8px 8px 0px 0px #000' }}>
            ...
            <p className="font-bold text-gray-900 mb-6">"{selectedLesson?.title}"</p>
            <p className="text-sm text-red-600 mb-6">⚠️ This action cannot be undone!</p>
            
            <div className="flex gap-3">
              <button
                onClick={() => {
                  setLessons(lessons.filter(l => l._id !== selectedLesson._id));
